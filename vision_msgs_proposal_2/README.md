...
ROS parameter. This is a topic of active discussion - it's possible
that the final form of the metadata database may take a completely different
form, such as an "object server" node. We currently expect a classifier to load
the database to the parameter
server in a manner similar to how URDFs are loaded and stored there (see [6]),
most likely defined in an XML format. The final solution will be concretely
defind using a ROS Enhancement Proposal, or REP [7].
...
## Messages
  ...
  * VisionInfo: Information about a vision pipeline, such as its name and where
...
can be fully represented are:
  ...
  * Pixel-wise segmentation into various classes, by using a `sensor_msgs/Image`
  message, where the pixel values index into the metadata database.
